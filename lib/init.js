...
function writeRollStatus(obj) {
  ...
  console.log(JSON.stringify(div_data))
...
function makeNewTableData(obj) {
  ...
  var score = ""
  var keylog = ""
  if (obj.score_label === "combo") {
    for (let i = 0; i < obj.yes_score.length; i++) {
      if (keylog.indexOf(obj.yes_score[i].toString()) === -1) {
        val = obj.roll_rack[obj.yes_score[i].toString()]
        key = obj.yes_score[i].toString()
        keylog += key
        label = nouns[val]
        label += (val > 2) ? " of a kind" : ""
        score = "0" //getRowScore()
        result.push({die_face: key, row_label: label, row_score: score})
      }
    }
  } else if (labels.indexOf(obj.score_label) > -1) {
    result.push({
      die_face: Object.keys(obj.roll_rack).join(","),
      row_label: obj.score_label,
      row_score: "0"
    })
  }
...
roll_button.addEventListener("click", () => {
  ...
  roll_status.results.score_log.push(makeNewTableData(roll_status.results))
