...
function writeRollStatus(obj) {
  ...
  function makeRowCells(data_arr, arg) {
    var shell = document.createElement("div")
    var div = {}
    var node = {}
    var index = (arg > 0) ? ( (arg > 1000) ? 1 : 0) : ( (arg < 0) ? 2 : 3 )
    var classes = [
      ["die_face","row_label","row_score"],
      ["total_face","total_label","total_score"],
      ["face_label","label_label","score_label"],
      ["roll_face","roll_label","roll_score"]
    ]
    var class_exts = [
      "",
      "",
    ...
    for (let i = 0; i < data_arr.length; i++) {
      ...
      node = document.createTextNode(data_arr[i])
      div.className = classes[index][i]
      div.appendChild(node)
      shell.appendChild(div)
    }
    shell.className = "row_throw" + class_exts[index]
    return shell
  }

  function makeRow(row_num, index_num, this_row) {
    var row_shell = document.createElement("div")
    var id_str = "row_" + index_num.toString() + "_" + row_num.toString()
    var row_cells = makeRowCells(
      [this_row.die_face,this_row.row_label,this_row.row_score], index_num
    )
    row_shell.appendChild(row_cells)
    row_shell.className = "flexOuterCenter"
    row_shell.id = "row_" + index_num.toString() + "_" + row_num.toString()
    return row_shell
  }

  function makeRowTag(int) {
    var shell = document.createElement("div")
    var tag = document.createElement("div")
    var text = document.createTextNode("roll " + int.toString())
    shell.className = "row_tag_shell"
    tag.className = "row_tag"
  ...
  var plural = (obj.score_log.length > 1) ? "s" : ""
  var turn_total = {
    die_face: "total",
    row_label: obj.score_log.length.toString() + " roll" + plural,
  ...
  var label_row = makeRow(-1,-1,table_labels)
  divs.push(label_row)
  score_shell.innerHTML = ""
  for (let i = 0; i < row_totals.length; i++) {
    roll_index++
    var tag_shell = makeRowTag(roll_index)
    divs.push(tag_shell)
  ...
  var turn_row = makeRow(1001,1001,turn_total)
  divs.push(roll_row)
  divs.push(turn_row)
...
function initScoreLogToggle(obj) {
  var index = 0
  console.log(obj.score_log.length)
  score_shell.childNodes.forEach( function (e) {
    if (e.className != "row_tag_shell") {
      if ( (Number(e.id[e.id.indexOf("_")+1]) > 0) &&
       (e.id[e.id.indexOf("_")+2] === "_") ) {
        index = Number(e.id[e.id.indexOf("_")+1])
        if (index === obj.score_log.length) {
          console.log("got latest roll - " + index)
          document.querySelector("#" + e.id).style.display = "block"
        } else {
          console.log("got prior roll - " + index)
...
function rackRolls(arr) {
  ...
  if (run_score.length === dice_rolls) {
    ...
    result.score_label = (result.test_score) ? "combo" : "farkle"
  }
...
var roll_status = {}
/*
roll status { results: { test_score: 0, yes_score: [], no_score: [] } }
*/
//DOM vars
...
roll_button.addEventListener("click", () => {
  ...
  initScoreLogToggle(roll_status.results)
})
...
re_roll_button.addEventListener("click", () => {
  ...
  roll_status.results.test_score += (new_score) ? new_score : -roll_status.results.test_score
