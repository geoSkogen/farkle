...
function highlightScoreDice(num_obj_arr, dom_obj_arr, bool) {
  var colors = ["firebrick","lightgrey","powderblue","limegreen","gold","orange","inidgo"]
  var threshold = (bool) ? 1 : 2
  var assoc_num_arr = {}
  var assoc_num_keys = []
  var key = ""
  var style_str = ""
  for (let i = 0 ; i < num_obj_arr.length; i++) {
    key = Object.keys(num_obj_arr[i])[0]
    assoc_num_arr[key] = num_obj_arr[i][key]
  }
  assoc_num_keys = Object.keys(assoc_num_arr)
  dom_obj_arr.forEach( (d) => {
    if (assoc_num_keys.indexOf(d.val.toString()) != -1) {
      key = assoc_num_keys[assoc_num_keys.indexOf(d.val.toString())]
      if (assoc_num_arr[key] > threshold) {
        style_str = "4px solid " + colors[assoc_num_arr[key]]
        d.el.style.border = style_str
      } else if (qualify_vals.indexOf(d.val) != -1) {
...
var roll_status = {}
              // 'results': { //logic  -  'dice': [ // DOM
              //    test_score : 0            { el: ('.diegram#one'),
              //    no_score : []               val: 3
              //    yes_score : []            }
              //  }                        . . . ]
//DOM vars
...
roll_button.addEventListener("click", () => {
  ...
  if (roll_status.results.yes_score) {
    disp_index = (roll_status.results.no_score) ? [0,1,1,0] : [0,0,0,1]
  }
  toggleDisplay(buttons,display_arr,disp_index)
  highlightScoreDice(roll_status.results.of_a_kind,roll_status.dice,allow_pairs)
...
re_roll_button.addEventListener("click", () => {
  ...
  toggleDisplay(buttons,display_arr,disp_index)
  highlightScoreDice(roll_status.results.of_a_kind,roll_status.dice,false)
  writeRollStatus(roll_status.results)
})
